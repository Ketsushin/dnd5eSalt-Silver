...
export async function getMigrationData() {
  const data = {};
  try {
    const icons = await fetch("systems/ketsushin-salt-silver/json/icon-migration.json");
    const spellIcons = await fetch("systems/ketsushin-salt-silver/json/spell-icon-migration.json");
...
function _migrateTokenImage(actorData, updateData) {
  ...
  for ( const path of ["texture.src", "prototypeToken.texture.src"] ) {
    ...
    if ( oldSystemPNG.test(v) ) {
      ...
      updateData[path] = `systems/ketsushin-salt-silver/tokens/${type}/${fileName}.webp`;
    }
  }
  return updateData;
}
