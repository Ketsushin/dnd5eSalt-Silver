...
export default class RollConfigurationDialog extends Dialog5e {
  ...
  static PARTS = {
    formulas: {
      template: "systems/ketsushin-salt-silver/templates/dice/roll-formulas.hbs"
    },
    configuration: {
      template: "systems/ketsushin-salt-silver/templates/dice/roll-configuration.hbs"
    },
    buttons: {
      template: "systems/ketsushin-salt-silver/templates/dice/roll-buttons.hbs"
    }
  };
  ...
  _identifyDiceTerms() {
    ...
    const identifyTerm = term => {
      ...
      for ( let i = 0; i < term.number; i++ ) dice.push({
        icon: `systems/ketsushin-salt-silver/icons/svg/dice/${term.denomination}.svg`,
