  ...
  buildTargetListEntry({ uuid, name }) {
    ...
    for ( const [change, values] of Object.entries(active) ) {
      ...
      for ( const type of values ) {
        if ( type === "ALL" ) {
          types.push({ type, change, icon: "systems/ketsushin-salt-silver/icons/svg/damage/all.svg" });
        } else {
  ...
  refreshListEntry(token, entry, options) {
    ...
    if ( thresholdButton && !active.threshold ) thresholdButton.remove();
      ...
      const button = this.getChangeSourceButton({
        change: "threshold", icon: "systems/ketsushin-salt-silver/icons/svg/damage/threshold.svg", type: "threshold"
